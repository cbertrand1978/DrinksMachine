...
namespace DrinksMachine.Logic.Executors
{
    ...
    public class DrinkExecutor : IServiceExecutor
    {
        ...
        /// </exception>
        public CommandResult ExecuteCommands(/*IBeverage beverage,*/ IBeverageTemplate template)
        {
            ...
            var drink = new Drink(template.Name);
            var result = new CommandResult(drink);

            foreach (var command in template.ServingActions)
            {
